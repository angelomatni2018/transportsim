...
bool isSmoothAndConnected(std::vector<Location> path) {
  ...
  for (auto i = 1; i < path.size(); ++i) {
    ...
    if (difference.first == 0 && difference.second == 0)
      return false;
    if (difference.first > 1 || difference.second > 1)
      return false;
  }
  return true;
}
    ...
    for (auto secondDir : directions) {
      ...
      for (auto thirdDir : directions) {
        ...
        if (!isSmoothAndConnected(path23)) {
          ...
          spdlog::trace("RoadSegmentTest: good path ({}) -> ({}) -> ({}) -> ({})", to_string(seg1.PrimaryLocation()),
                        to_string(seg2.PrimaryLocation()), to_string(seg3.PrimaryLocation()), to_string(seg4.PrimaryLocation()));
        }
        ++numGoodPaths;
      }
